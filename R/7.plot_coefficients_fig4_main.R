...
make_plot_data <- function(brm_model, nd_) {
  ...
  brms::posterior_epred(
    ...
    purrr::map_dfr(function(x, nd) {
      rbind(
        mean = apply (x,2,median),# colMeans(x),
...
mid_plots <- data.frame(
  ...
  SR_algae = 0, 
  SR_corals = 0,
  region = c("BrazilianCoastNorth","BrazilianCoastSouth","BrazilianOceanicIslands")
)
...
mid_pls <- ggplot(data = all_preds_mid, mapping = aes(x = region, y = mean)) +
  ...
  geom_errorbar(mapping = aes(ymin = `10%`, ymax = `90%`), colour = "grey60") +
  geom_point(mapping = aes(colour = group), lwd = 2, show.legend = FALSE) +
  scale_colour_manual(
    values = c(fish = "darkblue", algae = "darkgreen", corals = "darkorange")
  ) +
  labs(y = "", x = "Region") +
